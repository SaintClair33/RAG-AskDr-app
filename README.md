...
Ingest — the uploaded .txt file is read into memory.
Chunk — CharacterTextSplitter splits the text into 1,000-character chunks. Chunking is required because embedding models have a fixed context window, and because retrieval is more precise over small passages than over a whole document.
Embed — each chunk is converted to a vector with OpenAIEmbeddings, so that semantic similarity becomes a distance calculation.
Index — the vectors are loaded into a Chroma vector store.
Retrieve — the store is exposed as a retriever, which returns the chunks most similar to the user’s question.
Generate — a RetrievalQA chain (chain_type="stuff") inserts the retrieved chunks into the prompt and asks the LLM to answer from them.
